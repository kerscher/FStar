...
private let assumption'  :tactic unit =
  apply_raw (quote_lid ["FStar"; "Squash"; "return_squash"]);; assumption
 ...
 *)
private let assumption'' :tactic unit =
  or_else assumption' (apply_lemma (quote_lid ["Lang"; "lemma_addr_not_eq_refl"]);; norm [];; assumption')
...
private let simplify_unused_in :tactic unit =
  apply_lemma (quote_lid ["Lang"; "lemma_r_unused_in_minus"]) `or_else`
  apply_lemma (quote_lid ["Lang"; "lemma_r_unused_in_h"])     `or_else`
  return ()

private let simplify_contains_aux :tactic unit =
  assumption'' `or_else`
  apply_lemma (quote_lid ["Lang"; "lemma_contains_r_join_tot_restrict_minus"])     `or_else`
  apply_lemma (quote_lid ["Lang"; "lemma_contains_r_join_tot_points_to_minus"])    `or_else`
  apply_lemma (quote_lid ["Lang"; "lemma_contains_r1_join_tot_restrict_minus"])    `or_else`
  apply_lemma (quote_lid ["Lang"; "lemma_contains_r1_join_tot_points_to_minus"])   `or_else`
  apply_lemma (quote_lid ["Lang"; "lemma_contains_join_tot_h_emp_with_next_addr"]) `or_else`
  apply_lemma (quote_lid ["Lang"; "lemma_contains_r_points_to_unused_h"]);;
  split_all;;
  simplify_unused_in;;
  norm []
...
private let simplify_restrict_aux :tactic unit =
  apply_lemma (quote_lid ["Lang"; "lemma_eq_l_cong"]);;
  norm [];;
  apply_lemma (quote_lid ["Lang"; "lemma_restrict_r_join_tot_points_to_minus"])  `or_else`
  apply_lemma (quote_lid ["Lang"; "lemma_restrict_r_join_tot_restrict_minus"])   `or_else`
  apply_lemma (quote_lid ["Lang"; "lemma_restrict_r1_join_tot_restrict_minus"])  `or_else`
  apply_lemma (quote_lid ["Lang"; "lemma_restrict_r1_join_tot_points_to_minus"]) `or_else`
  apply_lemma (quote_lid ["Lang"; "lemma_restrict_join_tot_h_emp_with_next_addr"]);;
  norm [];;
  simplify_contains
...
private let step_bind :tactic unit =
  apply_lemma (quote_lid ["Lang"; "lemma_bind"]);;
  norm []
...
private let step_read_write :tactic unit =
  apply_lemma (quote_lid ["Lang"; "lemma_read_write"]);;  //lemma_read_write is the one that solves the frame
  norm [];;
  simplify_contains

private let step_alloc_return :tactic unit =
  apply_lemma (quote_lid ["Lang"; "lemma_alloc_return"]);;
  norm [];;
  simplify_contains
...
private let step_eq_implies_intro :tactic unit =
  apply_lemma (quote_lid ["Lang"; "lemma_eq_implies_intro"]);;
  norm []
...
private let simplify_select :tactic unit =
 apply_lemma (quote_lid ["Lang"; "lemma_sel_r_join_tot_restrict_minus"])     `or_else`
 apply_lemma (quote_lid ["Lang"; "lemma_sel_r_join_tot_points_to_minus"])    `or_else`
 apply_lemma (quote_lid ["Lang"; "lemma_sel_r1_join_tot_restrict_minus"])    `or_else`
 apply_lemma (quote_lid ["Lang"; "lemma_sel_r1_join_tot_points_to_minus"])   `or_else`
 apply_lemma (quote_lid ["Lang"; "lemma_sel_join_tot_h_emp_with_next_addr"]) `or_else`
 apply_lemma (quote_lid ["Lang"; "lemma_sel_join_tot_emp_with_next_addr_h"]);;
 simplify_contains
...
private let step_select :tactic unit =
 apply_lemma (quote_lid ["Lang"; "lemma_eq_cong"]);;
 norm [];;
 simplify_select;;
 trytac trefl;;
 return ()
